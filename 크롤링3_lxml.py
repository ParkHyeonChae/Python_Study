...
def main():
    response = requests.get("https://www.naver.com") # get으로 스크래핑 대상 url을 가져옴
...
def scrape_news_list_page(response): # 신문사 링크를 받는 함수 url부분을 파싱하는 코드
    ...
    for a in root.cssselect(".api_list .api_item a.api_link"): # css선택자 공부 필수, 네이버뉴스의 개발자도구의 a태그의 공통된 부분을 가져온다
                            # 뜻 : api list하위의 item 하위의 a태그의 api link를 가진애들을 싹 가져와라
        url = a.get("href") # href 속성 (개발자도구에서 확인)
        urls.append(url)
